...
class userToChallengeService {
  ...
  static async getUserChallengeInfo({ userId }) {
    const getJoinCallenge = await UserToChallenge.getJoinChallengeList({
      userId,
    });
    return getJoinCallenge;
  }
  // 포인트 조회
  //   static async getUserPointInfo({ userId }) {
  //     const getJoinCallenge = await UserToChallenge.getJoinPointList({
  //       userId,
  //     });
  //     return getJoinCallenge;
  //   }
