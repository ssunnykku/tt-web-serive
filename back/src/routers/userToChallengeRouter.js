...
userToChallengeRouter.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const userJoinChallenge = await userToChallengeService.createJoinedUser({
        userId,
        challengeId,
      });
  ...
  async function (req, res, next) {
    try {
      ...
      const userJoinChallengeList =
        await userToChallengeService.getUserChallengeInfo({ userId });
...
userToChallengeRouter.put(
  ...
  async function (req, res, next) {
    try {
      const userId = req.currentUserId;
      const challengeId = req.body.challengeId;
      const UserToChallenge = await userToChallengeService.JoinDelete({
        userId,
        challengeId,
      });
